...
# Nama file untuk penyimpanan data sementara
CLUSTER_DATA_FILE = "klasterisasi_result.csv"

# --- INISIALISASI SESSION STATE ---
# Pindahkan semua inisialisasi ke bagian paling atas
...
if 'current_menu' not in st.session_state:
    st.session_state.current_menu = "Unggah Data" # Set default menu
if 'kepsek_current_menu' not in st.session_state:
    st.session_state.kepsek_current_menu = "Lihat Hasil Klasterisasi" # Set default menu
...
def save_cluster_data(df_clustered, df_original, n_clusters, cluster_characteristics_map):
    """Saves clustering results to disk for persistence."""
    try:
        df_clustered.to_csv(CLUSTER_DATA_FILE, index=False)
        kepsek_data = {
            'n_clusters': n_clusters,
            'cluster_characteristics': cluster_characteristics_map,
            'df_original_str': df_original.to_csv(index=False)
        }
        pd.DataFrame([kepsek_data]).to_json("kepsek_data.json", orient='records')
        return True
    except Exception as e:
        st.error(f"Gagal menyimpan data persisten: {e}")
        return False

def load_cluster_data():
    """Loads clustering results from disk."""
    if os.path.exists(CLUSTER_DATA_FILE) and os.path.exists("kepsek_data.json"):
        try:
            df_clustered = pd.read_csv(CLUSTER_DATA_FILE)
            kepsek_data_df = pd.read_json("kepsek_data.json")
            kepsek_data = kepsek_data_df.iloc[0]
            
            # Re-create df_original from saved string
            df_original = pd.read_csv(io.StringIO(kepsek_data['df_original_str']))
            
            # Restore Kehadiran format
            if 'Kehadiran' in df_original.columns and df_original['Kehadiran'].dtype == 'object':
                df_original['Kehadiran'] = pd.to_numeric(df_original['Kehadiran'].str.rstrip('%').astype(float) / 100, errors='coerce')
                
            st.session_state.df_clustered = df_clustered
            st.session_state.df_original = df_original
            st.session_state.n_clusters = int(kepsek_data['n_clusters'])
            st.session_state.cluster_characteristics_map = kepsek_data['cluster_characteristics']
            return True
        except Exception as e:
            st.error(f"Gagal memuat data klasterisasi dari file. Silakan minta Operator TU untuk memproses ulang data. Error: {e}")
            return False
    return False
        ...
        if st.session_state.df_preprocessed_for_clustering is None or st.session_state.df_preprocessed_for_clustering.empty:
            ...
            if st.button("Jalankan Klasterisasi"):
                ...
                if df_clustered is not None:
                    ...
                    if save_cluster_data(df_final, st.session_state.df_original, k, st.session_state.cluster_characteristics_map):
                        st.success("Data klasterisasi berhasil disimpan untuk diakses oleh Kepala Sekolah.")
...
def show_kepala_sekolah_page():
    ...
    # Muat data persisten jika belum ada di session state
    if st.session_state.df_clustered is None:
        load_cluster_data()

    if st.session_state.df_clustered is None or st.session_state.df_clustered.empty:
        st.warning("Data klasterisasi tidak tersedia. Mohon Operator TU memproses data terlebih dahulu di menu 'Klasterisasi Data K-Prototypes'.")
        return

    # Logika untuk Kepsek, yang sekarang langsung membaca dari session_state
    if st.session_state.kepsek_current_menu == "Lihat Hasil Klasterisasi":
        ...
        if nama_terpilih_kepsek:
            ...
            with col_chart:
                ...
                values_siswa_plot_ekskul = [
                    float(siswa_data.get(col, 0)) * 100 for col in CATEGORICAL_COLS
                ]
                values_siswa_plot = values_siswa_plot_numeric + values_siswa_plot_ekskul
                fig, ax = plt.subplots(figsize=(10, 6))
                bars = sns.barplot(x=labels_siswa_plot, y=values_siswa_plot, palette="magma", ax=ax)
